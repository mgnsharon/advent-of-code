...
use crate::parser::{parse_directory_listing, parse_file_listing};
...
fn parse_dir(input: &str) -> IResult<&str, Files> {
    let (input, name) = parse_directory_listing(input)?;
...
fn parse_ls(input: &str) -> IResult<&str, Command> {
    ...
    let (input, files) = separated_list1(newline, alt((parse_dir, parse_file)))(input)?;
...
fn parse_cd(input: &str) -> IResult<&str, Command> {
    ...
    let cmd = match dir {
        "/" => Command::Cd(Cd::Root),
        ".." => Command::Cd(Cd::Up),
        name => Command::Cd(Cd::Down(name.to_string())),
    };
...
fn get_sizes(input: &str) -> BTreeMap<Vec<String>, u32> {
